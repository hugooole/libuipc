// Bazel entry point for wrecking_ball.
// Resolves module dir, tetmesh assets, and scene json via runfiles; calls
// uipc::init(); writes outputs to a cwd directory. Scene body matches main.cpp.
...
int main(int argc, char** argv)
{
    ...
    uipc_config["module_dir"]  = env.module_dir();
    init(uipc_config);

    std::string tetmesh_dir =
        env.folder_of("libuipc/assets/sim_data/tetmesh/cube.msh");
    std::string this_folder =
        env.folder_of("libuipc/apps/examples/wrecking_ball/wrecking_ball.json");
    auto this_output_path = env.output_dir("wrecking_ball_output");
    ...
    bazel_env::dump_config(config, this_output_path);
